import { push,getDatabase,ref } from '@firebase/database';
import { NativeBaseProvider,Center,  Card,Button, Select,
    VStack,
    CheckIcon,TextArea } from 'native-base';
import React,{useState} from 'react'
import { StyleSheet, Text,Image,ScrollView,localStorage, View } from 'react-native'
import { TextInput, TouchableOpacity } from 'react-native-gesture-handler'
import ImagePicker from 'react-native-image-crop-picker';
import { style } from 'styled-system';
import img1  from '../images/img1.jpg'

const CreateCatorgories = () => {


// const sellerData=localStorage.getItem("seller")
// console.log(sellerData)
let [service, setService] = React.useState("")
    const [img, setImage] = useState('data:image/jpeg;base64,')
    const [categoryName,setCategoryName]=useState('')
    const [categoryPrice,setPrice]=useState('')
    const [description,setDescription]=useState('')
    const [imgGallery1,setImageG1]=useState('data:image/jpeg;base64,')
    const [imgGallery2,setImageG2]=useState('data:image/jpeg;base64,')
    const [imgGallery3,setImageG3]=useState('data:image/jpeg;base64,')
    const openGallery1 = () => {
        ImagePicker.openPicker({
            width: 300,
            height: 400,
            cropping: true
        }).then(image => {
            console.log(image);
            setImageG1(image.path)
        });
    }
    const openGallery2 = () => {
        ImagePicker.openPicker({
            width: 300,
            height: 400,
            cropping: true
        }).then(image => {
            console.log(image);
            setImageG2(image.path)
        });
    }
    const openGallery3 = () => {
        ImagePicker.openPicker({
            width: 300,
            height: 400,
            cropping: true
        }).then(image => {
            console.log(image);
            setImageG3(image.path)
        });
    }


    const craeteCategories = () => {

        const db = getDatabase();
        push(ref(db, `seller/Categories/${service}` ), {
         categoryName:service,
        description:description,
           productName:categoryName,
           productPrice:categoryPrice,
           images:{
            img1:imgGallery1,
            img2:imgGallery2,
            img3:imgGallery3,
        },
        })
            .then(() => {
                // Data saved successfully!
                alert(" Data saved successfully!")
            })
            .catch((error) => {
                // The write failed...
                alert("The write failed...")
            });

    }
    return (
        <ScrollView>
        <NativeBaseProvider>
  
         <View>

     <Center>
                        <Text style={styles.heading}>Categories</Text>
                    </Center>
                    <VStack alignItems="center" style={styles.cat0} space={4}>
                        <Select style={styles.cat}
                            selectedValue={service}
                            minWidth="200"
                            accessibilityLabel="Choose Service"
                            placeholder="Choose Service"
                            _selectedItem={{
                                bg: "teal.600",
                                endIcon: <CheckIcon size="5" />,
                            }}
                            mt={1}
                            onValueChange={(itemValue) => setService(itemValue)}
                        >
                            <Select.Item label="Shoes" value="Shoes" />
                            <Select.Item label="Clothes" value="Clothes" />
                            <Select.Item label="Accessories" value="Accessories" />
                            <Select.Item label="Electronics" value="Electronics" />
                            <Select.Item label="Kids" value="Kids" />
                        </Select>
                    </VStack>
                    <View>
                        <TextInput style={styles.inp1} onChangeText={e => setCategoryName(e)} placeholder='Category Name' />
                        <TextInput style={styles.inp1} onChangeText={e => setPrice(e)} placeholder='Price' />
                        <TextInput style={styles.inp1} onChangeText={e => setDescription(e)} placeholder='Price' />
                            {/* <TextArea onChangeText={}  /> */}
                    </View>
    <Card style={styles.cards}>
                        <Card style={styles.card1}>
                            <Button style={styles.btnCard} onPress={openGallery1} >Image1</Button>
                            <Image source={{ uri: imgGallery1 }} resizeMode='contain' style={styles.catImges} />
                        </Card>
                        <Card style={styles.card1}>
                            <Button style={styles.btnCard} onPress={openGallery2}>Image1</Button>
                            <Image source={{ uri: img }} resizeMode='contain' style={styles.catImges} />
                        </Card>
                        <Card style={styles.card1}>
                            <Button style={styles.btnCard} onPress={openGallery3}>Image1</Button>
                            <Image source={{ uri: img }} resizeMode='contain' style={styles.catImges} />
                        </Card>
                    </Card>
         {/* <View style={styles.div1}>
 
         <TouchableOpacity style={styles.btn} onPress={openGallery}>
 
             <Text style={styles.btnText}>Select Image</Text>
 
         </TouchableOpacity>
         <View style={styles.imgBox}>
 
         <Image  source={{uri:img}} resizeMode='contain'  style={styles.imgBig} />
         </View>
         
         </View> */}
         {/* <View style={styles.div1}>
 
         <TouchableOpacity style={styles.btn} onPress={openGallery}>
 
             <Text style={styles.btnText}>Select Image</Text>
 
         </TouchableOpacity>
         <View style={styles.imgBox}>
 
         <Image  source={{uri:img}} resizeMode='contain'  style={styles.imgBig} />
         </View>
         
         </View> */}
         {/* <View style={styles.div1}>
 
         <TouchableOpacity style={styles.btn} onPress={openGallery}>
 
             <Text style={styles.btnText}>Select Image</Text>
 
         </TouchableOpacity>
         <View style={styles.imgBox}>
 
         <Image  source={{uri:img}} resizeMode='contain'  style={styles.imgBig} />
         </View>
         
         </View> */}
   
     
        
   
 
         {/* <Image source={}  /> */}
            <TouchableOpacity onPress={craeteCategories}  style={styles.btnCreate} >
                        <Text style={styles.btnCreateText}>Create Categories</Text>
                    </TouchableOpacity>
         </View>
             </NativeBaseProvider>
         </ScrollView>
    )
}

export default CreateCatorgories

const styles = StyleSheet.create({
    div1:{
        display:'flex',
        flexDirection:'row',
        justifyContent:'space-around',
        width:380,

        top:10,
        bottom:10,
        backgroundColor:'white',
        // right:100
        // borderWidth:2,
        // borderColor:'orange',
        left:5,
        marginTop:10

    },
    imgBig:{
        width:200,
        height:150,
        // left:110,
        
    },
    imgBox:{
        // borderWidth:1,
        // borderColor:'black',
        width:200,
        right:10,

    },
    cat:{
borderWidth:1,
borderColor:'orange'
    },
    

    btn:{
        width:120,
        height:50,
        top:40,
        backgroundColor:'orange'
    },
    btnText:{
        top:15,
        left:20,
        color:'white'
    },
    inp1:{
        marginTop:20,
        marginBottom:10,
        borderColor:'orange',
        borderWidth:1,
        width:350,
        left:20,
        borderRadius:10


    },
    text1:{
        marginTop:20,
        marginBottom:10,
        borderColor:'orange',
        borderWidth:1,
        width:350,
        height:100,
        left:20,
        borderRadius:10

    },
    btnCreate:{
        width:150,
        height:40,
        backgroundColor:'green',
        left:120,
        bottom:5,
        top:20,
        marginBottom:40,
        borderRadius:10
    },
    btnCreateText:{
        left:20,
        top:10,
        color:'white'
    },
    heading:{
        fontSize:30,
        top:10,
       color:'black'

    },
    cards:{
        display:'flex',
        flexDirection:'row',
        justifyContent:'space-around'
    },
    catImges:{
        width:90,
        height:80,
        right:5
    },
    cards:{
        height:160,
        // top:30,
        display:'flex',
        justifyContent:'space-around',
        flexDirection:'row'
    },
    btnCard:{
        width:90,
        right:6,
      
    },
    card1:{
        width:110
    },
    cat0:{
        top:10
    }

})

import { TextArea,NativeBaseProvider } from 'native-base';
import React,{useState} from 'react'
import { StyleSheet,Center, Text,Image,ScrollView, View } from 'react-native'
import { TextInput, TouchableOpacity } from 'react-native-gesture-handler'
import ImagePicker from 'react-native-image-crop-picker';
import { getDatabase, ref, push } from "firebase/database";
import img1  from '../images/img1.jpg'
const CreateProducts = () => {

    const [img, setImage] = useState('data:image/jpeg;base64,')
    const [img2, setImage2] = useState('data:image/jpeg;base64,')
    const [img3, setImage3] = useState('data:image/jpeg;base64,')
    // const [productName ,setProductName]=useState('')
    // const [productPrice ,setProductPrice]=useState('')
    // const [productDescription ,setProductDescription]=useState('')
    const [productName,setProduct]=useState('')
    const [productPrice,setPrice]=useState('')
    const [description,setDescription]=useState('')
    const createProduct = ({navigation}) => {

    //   alert(productName,productDescription)
        const db = getDatabase();
        push(ref(db, 'seller/allProducts'), {
          images:{
              img1:img,
              img2:img2,
              img3:img3
          },
          price:productPrice,
          name:productName,
          description:description
        })
            .then(() => {
                // Data saved successfully!
                alert('Successfully Created Product')
            })
            .catch((error) => {
                alert('Sorry....!')
            });

    }
    const openGallery1 = () => {
        ImagePicker.openPicker({
            width: 300,
            height: 400,
            cropping: true
        }).then(image => {
            console.log(image);
            setImage(image.path)
        });
    }
    const openGallery2 = () => {
        ImagePicker.openPicker({
            width: 300,
            height: 400,
            cropping: true
        }).then(image => {
            console.log(image);
            setImage2(image.path)
        });
    }
    const openGallery3 = () => {
        ImagePicker.openPicker({
            width: 300,
            height: 400,
            cropping: true
        }).then(image => {
            console.log(image);
            setImage3(image.path)
        });
    }
    // const openCamer=()=>{
    //     ImagePicker.openCamera({
    //         width: 300,
    //         height: 400,
    //         cropping: true,
    //       }).then(image => {
    //         console.log(image);
            
    //       });
    // }
    return (
        <ScrollView>
        <NativeBaseProvider>
            <View>
                {/* <Center>
                   <Text> Create Product</Text>
                    </Center> */}
                <TouchableOpacity  style={styles.btnLog} onPress={()=>navigation.navigate('Login')}>    
                    <Text>Logout</Text>
                </TouchableOpacity>
                
                <View style={styles.div1}>
                    <TouchableOpacity style={styles.btn} onPress={openGallery1}>

                        <Text style={styles.btnText}>OPEN</Text>

                    </TouchableOpacity>
                    <View style={styles.imgBox}>

                        <Image source={{ uri: img }} resizeMode='contain' style={styles.imgBig} />
                    </View>

                </View>
                <View style={styles.div1}>
                    <TouchableOpacity style={styles.btn} onPress={openGallery2}>

                        <Text style={styles.btnText}>OPEN</Text>

                    </TouchableOpacity>
                    <View style={styles.imgBox}>

                        <Image source={{ uri: img2 }}    resizeMode='contain' style={styles.imgBig} />
                    </View>

                </View>
                <View style={styles.div1}>
                    <TouchableOpacity style={styles.btn} onPress={openGallery3}>

                        <Text style={styles.btnText}>OPEN</Text>

                    </TouchableOpacity>
                    <View style={styles.imgBox}>

                        <Image source={{ uri: img3 }}  resizeMode='contain' style={styles.imgBig} />
                    </View>

                </View>
                <View>
                    <TextInput style={styles.inp1} onChangeText={e => setProduct(e)} placeholder='Product Name' />
                    <TextInput style={styles.inp1}  onChangeText={e => setPrice(e)} placeholder='Product Price' />
                    <TextArea style={styles.text1}  onChangeText={e => setDescription(e)} placeholder='Product Descriptions' />
                </View>

                {/* <Image source={}  /> */}
                <TouchableOpacity style={styles.btnCreate} onPress={createProduct}  >
                    <Text style={styles.btnCreateText} >Create Product</Text>
                </TouchableOpacity>
            </View>
        </NativeBaseProvider>
    </ScrollView>
    )
}

export default CreateProducts

const styles = StyleSheet.create({
    div1:{
        display:'flex',
        flexDirection:'row',
        justifyContent:'space-around',
        width:380,

        top:10,
        bottom:10,
        backgroundColor:'white',
        // right:100
        borderWidth:2,
        borderColor:'orange',
        left:5,
        marginTop:10

    },
    imgBig:{
        width:200,
        height:150,
        // left:110,
        
    },
    imgBox:{
        // borderWidth:1,
        // borderColor:'black',
        width:200,
        right:10,

    },
    btn:{
        width:120,
        height:50,
        top:40,
        backgroundColor:'orange'
    },
    btnText:{
        top:15,
        left:40,
        color:'white'
    },
    inp1:{
        marginTop:20,
        marginBottom:10,
        borderColor:'orange',
        borderWidth:1,
        width:350,
        left:20


    },
    text1:{
        marginTop:20,
        marginBottom:10,
        borderColor:'orange',
        borderWidth:1,
        width:350,
        height:100,
        left:20,
        borderRadius:10

    },
    btnCreate:{
        width:150,
        height:40,
        backgroundColor:'green',
        left:120,
        bottom:5,
        borderRadius:10
    },
    btnCreateText:{
        left:25,
        top:10,
        color:'white'
    }

})

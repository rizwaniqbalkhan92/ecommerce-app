import { TextArea, NativeBaseProvider } from 'native-base';
import React, { useState } from 'react'
import { StyleSheet, Text, Image, ScrollView, View } from 'react-native'
import { TextInput, TouchableOpacity } from 'react-native-gesture-handler'
import ImagePicker from 'react-native-image-crop-picker';
import img1 from '../images/img1.jpg'
const CreateProducts = () => {

    const [img, setImage] = useState('data:image/jpeg;base64,')
    const [img2, setImage2] = useState('data:image/jpeg;base64,')
    const [img3, setImage3] = useState('data:image/jpeg;base64,')
    const [productName ,setProductName]=useState('')
    const [setProductPrice ,setProductPrice]=useState('')
    const [productDescription ,setProductDescription]=useState('')

    const createProduct = () => {

        const db = getDatabase();
        set(ref(db, 'users/' + userId), {
            username: name,
            email: email,
            profile_picture: imageUrl
        })
            .then(() => {
                // Data saved successfully!
            })
            .catch((error) => {
                // The write failed...
            });

    }
    const openGallery = () => {
        ImagePicker.openPicker({
            width: 300,
            height: 400,
            cropping: true
        }).then(image => {
            console.log(image);
            setImage(image.path)
        });
    }
    // const openCamer=()=>{
    //     ImagePicker.openCamera({
    //         width: 300,
    //         height: 400,
    //         cropping: true,
    //       }).then(image => {
    //         console.log(image);

    //       });
    // }
    return (
        <ScrollView>
            <NativeBaseProvider>
                <View>
                    <View style={styles.div1}>
                        <TouchableOpacity style={styles.btn} onPress={openGallery}>

                            <Text style={styles.btnText}>OPEN</Text>

                        </TouchableOpacity>
                        <View style={styles.imgBox}>

                            <Image source={{ uri: img }} onChangeText={e=>setImage(e.target.value)} resizeMode='contain' style={styles.imgBig} />
                        </View>

                    </View>
                    <View style={styles.div1}>
                        <TouchableOpacity style={styles.btn} onPress={openGallery}>

                            <Text style={styles.btnText}>OPEN</Text>

                        </TouchableOpacity>
                        <View style={styles.imgBox}>

                            <Image source={{ uri: img2 }}  onChangeText={e=>setImage2(e.target.value)}  resizeMode='contain' style={styles.imgBig} />
                        </View>

                    </View>
                    <View style={styles.div1}>
                        <TouchableOpacity style={styles.btn} onPress={openGallery}>

                            <Text style={styles.btnText}>OPEN</Text>

                        </TouchableOpacity>
                        <View style={styles.imgBox}>

                            <Image source={{ uri: img3 }}  onChangeText={e=>setImage3(e.target.value)} resizeMode='contain' style={styles.imgBig} />
                        </View>

                    </View>
                    <View>
                        <TextInput style={styles.inp1} onChangeText={e=>setProductName(e.target.value)} placeholder='Product Name' />
                        <TextInput style={styles.inp1}  onChangeText={e=>setProductPrice(e.target.value)} placeholder='Product Price' />
                        <TextArea style={styles.text1}  onChangeText={e=>setProductDescription(e.target.value)} placeholder='Product Descriptions' />
                    </View>

                    {/* <Image source={}  /> */}
                    <TouchableOpacity style={styles.btnCreate}>
                        <Text style={styles.btnCreateText} onPress={createProduct}  >Create Product</Text>
                    </TouchableOpacity>
                </View>
            </NativeBaseProvider>
        </ScrollView>
    )
}

export default CreateProducts

const styles = StyleSheet.create({
    div1: {
        display: 'flex',
        flexDirection: 'row',
        justifyContent: 'space-around',
        width: 380,

        top: 10,
        bottom: 10,
        backgroundColor: 'white',
        // right:100
        borderWidth: 2,
        borderColor: 'orange',
        left: 5,
        marginTop: 10

    },
    imgBig: {
        width: 200,
        height: 150,
        // left:110,

    },
    imgBox: {
        // borderWidth:1,
        // borderColor:'black',
        width: 200,
        right: 10,

    },
    btn: {
        width: 120,
        height: 50,
        top: 40,
        backgroundColor: 'orange'
    },
    btnText: {
        top: 15,
        left: 40,
        color: 'white'
    },
    inp1: {
        marginTop: 20,
        marginBottom: 10,
        borderColor: 'orange',
        borderWidth: 1,
        width: 350,
        left: 20


    },
    text1: {
        marginTop: 20,
        marginBottom: 10,
        borderColor: 'orange',
        borderWidth: 1,
        width: 350,
        height: 100,
        left: 20,
        borderRadius: 10

    },
    btnCreate: {
        width: 150,
        height: 40,
        backgroundColor: 'green',
        left: 120,
        bottom: 5,
        borderRadius: 10
    },
    btnCreateText: {
        left: 25,
        top: 10,
        color: 'white'
    }

})
